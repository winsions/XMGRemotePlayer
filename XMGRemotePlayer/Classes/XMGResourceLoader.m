...
@interface XMGResourceLoader ()<XMGAudioDownLoaderDelegate>
...
- (void)handleAllRequest {
    ...
    for (AVAssetResourceLoadingRequest *loadingRequest in self.loadingRequests) {
        ...
        NSData *data = [NSData dataWithContentsOfFile:[XMGAudioFileTool tmpPathWithURL:url] options:NSDataReadingMappedIfSafe error:nil];
        if (data.length == 0) {
            data = [NSData dataWithContentsOfFile:[XMGAudioFileTool cachePathWithURL:url] options:NSDataReadingMappedIfSafe error:nil];
        }
...
- (void)handleRequestWithLoadingRequest: (AVAssetResourceLoadingRequest *)loadingRequest {
    NSURL *url = [loadingRequest.request.URL httpURL];
    // 1. 填充信息头
    loadingRequest.contentInformationRequest.contentType = [XMGAudioFileTool contentTypeWithURL:url];
    loadingRequest.contentInformationRequest.contentLength = [XMGAudioFileTool cacheFileSizeWithURL:url];
    loadingRequest.contentInformationRequest.byteRangeAccessSupported = YES;
    
    
    // 2. 响应数据
    NSData *data = [NSData dataWithContentsOfFile:[XMGAudioFileTool cachePathWithURL:url] options:NSDataReadingMappedIfSafe error:nil];
