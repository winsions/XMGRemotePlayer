...
@property (nonatomic, strong) XMGResourceLoader *resourceLoader;
...
+ (instancetype)shareInstance {
    if (!_shareInstance) {
        _shareInstance = [[XMGRemotePlayer alloc] init];
    }
...
- (void)playWithURL: (NSURL *)url isCache:(BOOL)isCache {
    ...
    if ([self.url isEqual:url]) {
        ...
        if (self.state == XMGRemotePlayerStatePlaying) {
            return;
        }
        if (self.state == XMGRemotePlayerStatePause) {
            [self resume];
            return;
        }
        if (self.state == XMGRemotePlayerStateLoading) {
            return;
        }
    ...
    self.resourceLoader = [[XMGResourceLoader alloc] init];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
    ...
    if ([keyPath isEqualToString:@"status"]) {
        ...
        switch (status) {
            ...
            {
                NSLog(@"未知");
                self.state = XMGRemotePlayerStateUnknown;
                break;
            }
        }
